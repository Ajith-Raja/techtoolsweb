    ...
    async def send_progress(self, task_id: str, data: Dict[str, Any]):
        ...
        if websocket:
            try:
                ...
                del self.active_connections[task_id]  # Clean up dead socket
...
async def update_progress(task_id: str, progress: int, status: str = "processing", error: str = None, file: str = None):
...
async def process_pdf_to_word(task_id: str, pdf_file: bytes):
    ...
    try:
        ...
        await update_progress(task_id, 5)
        ...
        try:
            ...
            # Convert page by page to track progress
            cv = Converter(input_path)
            
            # Instead of converting page by page (which might cause issues),
            # we'll convert the entire document but simulate progress updates
            await update_progress(task_id, 15)
            
            # Convert the entire PDF
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: cv.convert(output_path, start=0, end=None)
            )
            
            cv.close()
            
            # Simulate progress updates for each page
            for i in range(total_pages):
                progress = 15 + int(80 * (i + 1) / total_pages)
                await update_progress(task_id, progress)
